...
/**
 * Lista apresentacional de jogos AO VIVO (abas por esporte + busca + jogos por
 * liga). Agnóstica de FONTE: recebe `games/loading/error/live` de qualquer hook
 * (fssb/SSE ou Altenar/polling) e `onOpen` decide a navegação. Reusada por
 * LiveGamesList (fssb) e pela lista biahosted.
 */
...
interface Props {
  ...
  live: boolean;
  emptyLabel: string;
  onOpen: (game: LiveGame) => void;
}

export function LiveGamesView({ games, loading, error, live, emptyLabel, onOpen }: Props) {
        ...
        leagues.map((b) => (
          <div key={b.name}>
            ...
            <div className="grid gap-1.5 sm:grid-cols-2 xl:grid-cols-3">
              {b.games.map((g) => (
                <GameRow key={g.id} game={g} onOpen={() => onOpen(g)} />
              ))}
            </div>
          </div>
        ))
